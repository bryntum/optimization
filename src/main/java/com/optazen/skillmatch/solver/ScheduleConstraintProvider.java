...
public class ScheduleConstraintProvider implements ConstraintProvider {
    ...
    protected Constraint overtime(ConstraintFactory constraintFactory) {
        return constraintFactory.forEach(Event.class)
                ...
                .penalize(HardMediumSoftScore.ofHard(1), (event, constraintParameters) -> Math.toIntExact(ChronoUnit.HOURS.between(LocalDateTime.of(event.getStartDate().toLocalDate(), constraintParameters.endTime()), event.getEndDate())))
    ...
    protected Constraint unassigned(ConstraintFactory constraintFactory) {
        return constraintFactory.forEachIncludingUnassigned(Event.class)
                .filter(event -> event.getResource() == null)
                .penalize(HardMediumSoftScore.ofMedium(1), Event::getDuration)
